		...
		actions: {
			...
			loadSomeData: () => {
				...
				fetch("https://www.swapi.tech/api/people?page=2&limit=10")
					.then(response => response.json())
					.then(async data => {
						let personsBasic = data.results;
						let personsFull = personsBasic.map(async elem => {
							const datos = await fetch(elem.url).then(response => response.json());
							return {
								name: elem.name,
						...
						let personsFullFinal = await Promise.all(personsFull);
						setStore({ persons: personsFullFinal });
					});
			}
		}
	};
